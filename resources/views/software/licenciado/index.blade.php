...
@section('title', 'Software licenciado')
@section('page_title', 'Software licenciado')
...
<div x-data="{ q: '' }"
    ...
    <div class="mb-4 flex flex-wrap items-center gap-3">
        ...
        @if(auth()->user()->esAdmin())
            <a href="{{ route('softlic.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-[#b3072d] px-4 py-2.5 text-sm font-bold text-white shadow-lg shadow-red-900/15 transition hover:bg-[#8e0626]"><i class="bi bi-plus-lg"></i>Agregar software licenciado</a>
        @endif
        <span class="ml-auto rounded-full bg-brand-50 px-3 py-1.5 text-xs font-bold text-brand-700">{{ $software->total() }} producto(s)</span>
    </div>
            ...
            <table class="w-full min-w-[760px] text-left text-sm">
                <thead>
                    <tr class="border-b border-slate-100 bg-slate-50/70 text-[11px] uppercase tracking-wider text-slate-400">
                        @foreach(['ID', 'Nombre', 'Clave', 'Tipo', 'Versión', 'Licenciamiento', 'Uso'] as $h)
                ...
                <tbody x-ref="rows" class="divide-y divide-slate-50">
                    @forelse($software as $sw)
                        @php($asignadas = $sw->hardwares->count())
                        @php($agotada = $sw->Software_Cantidad !== null && $asignadas >= $sw->Software_Cantidad)
                        <tr data-row class="group transition hover:bg-brand-50/40">
                            <td class="px-5 py-3.5 font-mono text-xs font-bold text-slate-400">#{{ $sw->Software_Id }}</td>
                            <td class="px-5 py-3.5 font-semibold text-slate-700">{{ $sw->Software_Nombre ?? '(sin nombre)' }}</td>
                            <td class="px-5 py-3.5"><code class="rounded-lg bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-600">{{ $sw->Software_Clave ?? '(sin clave)' }}</code></td>
                            <td class="px-5 py-3.5 text-slate-500">{{ $sw->tipoSoftware->Soft_Tipo_Nombre ?? '—' }}</td>
                            <td class="px-5 py-3.5 text-slate-500">{{ $sw->Software_Version ?? '—' }}</td>
                            <td class="px-5 py-3.5 text-slate-500">{{ $sw->Software_Tip_Licenciamiento ?? '—' }}</td>
                            <td class="px-5 py-3.5">
                                @if($sw->Software_Cantidad !== null)
                                    <span class="inline-flex h-6 w-14 items-center justify-center rounded-lg text-xs font-bold {{ $agotada ? 'bg-red-50 text-red-600 ring-1 ring-red-200' : 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200' }}">{{ $asignadas }}/{{ $sw->Software_Cantidad }}</span>
                                @else
                                    <span class="inline-flex h-6 w-14 items-center justify-center rounded-lg bg-sky-50 text-xs font-bold text-sky-700 ring-1 ring-sky-200">{{ $asignadas }}</span>
                                @endif
                            </td>
                            <td class="px-5 py-3.5">
                                <div class="flex items-center justify-end gap-1 opacity-60 transition group-hover:opacity-100">
                                    <a href="{{ route('softlic.show', $sw->Software_Id) }}" title="Ver" class="inline-grid size-8 place-items-center rounded-lg text-slate-400 transition hover:bg-brand-50 hover:text-brand-600"><i class="bi bi-eye"></i></a>
                                    @if(auth()->user()->esAdmin())
                                        <a href="{{ route('softlic.edit', $sw->Software_Id) }}" title="Editar" class="inline-grid size-8 place-items-center rounded-lg text-slate-400 transition hover:bg-amber-50 hover:text-amber-600"><i class="bi bi-pencil"></i></a>
                                    @endif
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="8"><x-empty-state icono="bi-key" texto="{{ $q !== '' ? 'Sin resultados.' : 'Aún no hay licencias registradas.' }}" accion="softlic.create" boton="Nueva licencia" /></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
